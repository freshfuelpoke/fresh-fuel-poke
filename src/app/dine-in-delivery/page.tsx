...
export default async function DineInDeliveryPage() {
  ...
  const dineInHours = hours.find(h => h.label.toLowerCase().includes('dine'))?.value || (hours.length > 0 ? hours[0].value : "");
  const takeAwayHours = hours.find(h => h.label.toLowerCase().includes('take'))?.value || (hours.length > 1 ? hours[1].value : (hours.length > 0 ? hours[0].value : ""));
    ...
    <main className="bg-white text-[#121212]">
      ...
      <section className="px-6 py-12 md:px-12 md:py-16 lg:px-24 overflow-hidden">
        <Reveal className="mx-auto custom-container max-w-6xl" delay={80}>
          <div>
            <h1 className="font-serif text-3xl md:text-4xl mb-6 text-[#1a1a1a] tracking-tight">Dine-in & Delivery</h1>
              ...
              <div className="space-y-12">
                ...
                <div className="relative">
                  <div className="flex items-end justify-between mb-3">
                    <h3 className="font-serif text-2xl text-stone-500">Dine-In</h3>
                    <div className="flex flex-col items-end">
                      <div className="flex items-center gap-1.5">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-[var(--lime)]"><path d="M3 2v7c0 1.1.9 2 2 2h4a2 2 0 0 0 2-2V2"></path><path d="M7 2v20"></path><path d="M21 15V2v0a5 5 0 0 0-5 5v6c0 1.1.9 2 2 2h3Zm0 0v7"></path></svg>
                        <span className="text-sm font-semibold text-stone-900">Dine In</span>
                      </div>
                      <span className="text-stone-400 text-xs mt-0.5">{dineInHours}</span>
                    </div>
                  </div>
                  <div className="h-[2px] w-full bg-stone-200 mb-6"></div>
                  
                  <div className="space-y-6">
                    <h4 className="text-stone-500 font-medium text-sm tracking-wide">Dine-in Service</h4>
                    ...
                    <div className="space-y-1.5 pt-2">
                      {restaurantInfo?.phone && (
                        <p className="text-stone-500 text-sm">
                          ...
                          <a href={`tel:${restaurantInfo.phone}`} className="hover:text-stone-800 transition-colors">{restaurantInfo.phone}</a>
                        </p>
                      )}
                      {restaurantInfo?.whatsApp && (
                        <p className="text-stone-500 text-sm">
                          <span className="font-light">Whatsapp:</span>{" "}
                          <a href={`https://wa.me/${restaurantInfo.whatsApp.replace(/[^\d]/g, "")}`} target="_blank" rel="noopener noreferrer" className="hover:text-stone-800 transition-colors">{restaurantInfo.whatsApp}</a>
                        </p>
                      )}
                    </div>
                  </div>
                ...
                <div className="relative">
                  <div className="flex items-end justify-between mb-3">
                    <h3 className="font-serif text-2xl text-stone-500">Take Away</h3>
                    <div className="flex flex-col items-end">
                      <div className="flex items-center gap-1.5">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-[var(--brand-red)]"><path d="M6 2L3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"></path><line x1="3" y1="6" x2="21" y2="6"></line><path d="M16 10a4 4 0 0 1-8 0"></path></svg>
                        <span className="text-sm font-semibold text-stone-900">Take Away</span>
                      </div>
                      <span className="text-stone-400 text-xs mt-0.5">{takeAwayHours}</span>
                    </div>
                  </div>
                  <div className="h-[2px] w-full bg-stone-200 mb-6"></div>
